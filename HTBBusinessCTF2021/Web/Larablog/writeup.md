...
The script requires the APP_KEY and a payload.

The supplied payload should be a malicious PHP object that will execute your command when deserialized.

The script forges a new cookie with the payload inside. After that the cookie is encrypted with the given APP_KEY.

Fortunately there are pre-built payloads that can be generated with tools such as [PHPGGC](https://github.com/ambionics/phpggc).
